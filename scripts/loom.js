...
function stripHtml(html) {
    ...
    _temp_Div.innerHTML = html;
    return _temp_Div.textContent || _temp_Div.innerText || "";
}

function replaceExtension(){

    //for job_image
    let job_holder_company_img_prop = document.querySelectorAll('[data-testid="simpler-left-logo"]')[0];
    let job_holder_company_img = job_holder_company_img_prop !== undefined ? job_holder_company_img_prop.querySelector('img').src : "https://earnest-medovik-6139b7.netlify.app/greyed.png";

    // for job url
    let job_holder_job_url = window.location.href;

    //for name, location, company
    let job_holder_i = document.querySelector('.jobsearch-InfoHeaderContainer');

    let job_holder_iprop = job_holder_i.querySelector('[data-testid="simpler-simplified-header"]') || job_holder_i.firstChild;

    let job_holder_job_name_prop = job_holder_iprop.querySelector('[data-testid="simpler-jobTitle"]') || job_holder_iprop.querySelector('[data-testid="jobsearch-JobInfoHeader-title"]') || document.querySelector('[data-testid="jobsearch-JobInfoHeader-title"]');
    let job_holder_job_name = stripHtml(job_holder_job_name_prop.innerHTML).replace("- job post","");
    
    let job_holder_company_name_prop = document.querySelector('[data-testid="inlineHeader-companyName"]') || document.querySelector('[data-testid="simpler-simplified-header"]').querySelectorAll('a')[0] || job_holder_iprop.querySelector('.jobsearch-JobInfoHeader-companyNameSimple') || job_holder_iprop.querySelector('[data-testid="inlineHeader-companyName"]').querySelectorAll('a')[0];
    let job_holder_company_name = stripHtml(job_holder_company_name_prop.innerHTML);

    let job_holder_job_location_prop = job_holder_iprop.querySelector('[data-testid="jobsearch-JobInfoHeader-companyLocation"]') || document.querySelector('[data-testid="inlineHeader-companyLocation"]') || document.querySelector('[data-testid="job-location"]');
    let job_holder_job_location = stripHtml(job_holder_job_location_prop.innerHTML);

    //for job description
    let job_holder_job_description = document.querySelector('#jobDescriptionText').innerHTML;
    ...
    cannonierTabImg.alt = 'https://earnest-medovik-6139b7.netlify.app/greyed.png';
    ...
    // Assuming you want to append these to the body or a specific element
...
function triggers(){
    ...
    cannonier_save.addEventListener('click', checkUserCookie);
};
...
function complextriggers(){
    ...
    cannonier_save.addEventListener('click', checkUserCookie_Action);
...
const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
        if (mutation.type === 'childList') {
            const jobsConf = document.querySelector('.jobsearch-RightPane');
            const jobsHome = document.querySelector('.jobsearch-JobComponent');
            if (jobsConf || jobsHome) {
                if(once == false){
                    // console.log('Added');
                    isset = true;
                    once = true;
...
const SkeletonObserver = new MutationObserver(mutationsList => {
    mutationsList.forEach(mutation => {
        if(mutation.type === 'childList') {
            const removedNodes = Array.from(mutation.removedNodes);
            const skeletonDivRemoved = removedNodes.some(node => node.nodeType === Node.ELEMENT_NODE && node.getAttribute('data-testid') === 'viewJob-skeleton');
            if(skeletonDivRemoved) {
                removeFullExtension();
                replaceExtension();
            }
        }
...
function checkDesc(){
    if(document.querySelector('#job-full-details') || document.querySelector('.jobsearch-JobComponent')){
        // alert('Found');
        removeFullExtension();
        replaceExtension();
        SkeletonObserver.observe(targetNode, config);
    }
    // else{
    //     alert('Not Found');
    // }
...
// window.addEventListener('load', function(){
    // chrome.runtime.sendMessage({ message: "Need" }, (response) => {
    //     state = !state;
    // });
// });
